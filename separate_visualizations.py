...
class SeparateVisualizationSystem:
    ...
    def load_realistic_data(self):
        """Load the realistic unified weather scores."""
        try:
            scores_path = '/Users/vasubansal/code/universal_atmospheric_model/data/predictions/realistic_unified_scores.csv'
            self.unified_scores = pd.read_csv(scores_path)
            print(f"Loaded {len(self.unified_scores)} planet scenario scores")
            return True
        except FileNotFoundError:
            print("❌ Realistic scores not found. Run realistic_weather_system.py first.")
            return False
